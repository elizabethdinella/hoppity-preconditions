    ...
    if isinstance(json_node, dict):
        ...
        for d in json_node:
            if isinstance(d, dict):
                ch_node = build_java_ast_from_json(d, None, ast)
            else:
                assert d is None
...
def build_java_ast(fname):
    with open(fname, 'r') as f:
        ...
        ast = AST()
        root = build_java_ast_from_json(root_node, None, ast)
        ast.root_node = root
    return ast
